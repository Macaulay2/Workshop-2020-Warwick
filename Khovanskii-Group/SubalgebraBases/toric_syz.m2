...
intrinsicReduce(Subring, Matrix, RingElement) := (subR, G, p) -> (
    ...
    -- This will only actually be the initial algebra if G is a GB.
...
intrinsicBuchberger(Subring, Matrix) := (subR, S) -> (
	...
	M := (toricSyz(subR, C));
     	...
     	-- Verify that M the columns of M are syzygies of LT(C).
	
	assert(M*(transpose leadTerm fullSub C)==0);
	print("Total syzygies: "|toString(numcols M));
    	newGens := for i from 0 to (numcols M) - 1 list(
	    H := (M^{i} // subR);
	    -- This assertion is condition 4.1. (Note: matrix equality is broken)
	    --assert( (leadTerm fullSub H) - (M^{i}) == 0);
	    f := trace ((diagonalMatrix H)*(diagonalMatrix currentGens));
	    ...
	    hbar = hbar//subR;
	    if hbar == 0 or hbar == 1 then(
		print("hbar constant");
		);
	    print("New");
	    print("number of terms:"|toString(numcols monomials hbar));
	    print("degree:"|toString(degree hbar));
	    print("Lead term:");
	    print(leadTerm hbar);
	    hbar
	    );
	print("-- total new generators:"|toString(length newGens));
	    ...
	    currentGens = currentGens | matrix({newGens});
	    );
    	loopNum = loopNum + 1;
        );
...
toricSyz(Subring, Matrix) := (subR, M) -> (
    ...
    -- We can do this because we know subR is a Sagbi basis.  
    KA := subring leadTerm gens subR;
    ...
    -- Discard the degree information because it breaks the matrix equality operators...
