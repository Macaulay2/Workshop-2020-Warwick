...
doc ///
    ...
    Description        
     Text
      {\bf Graphical Models MLE} is a package for algebraic statistics that broadens the functionalities of {\bf Graphical Models}. 
      It computes the maximum likelihood estimates (MLE) of the covariance matrix of 
      continuous Gaussian graphical models.
      
      The main features of the package are the computation of the @TO sampleCovarianceMatrix@ of sample data, 
      the @TO scoreEquations@ of log-likelihood functions of continuous Gaussian graphical models,
      the @TO MLdegree@ of the model and the MLE for the covariance or concentration matrix via @TO solverMLE@.
      
      An introduction to key algebraic statistics notions can be found in the books:
      ...
      We consider graphical models coming from loopless mixed graphs (LMG), see the paper:  
      Kayvan Sadeghi and Steffen Lauritzen, 2014. {\em Markov properties for mixed graphs}, Bernoulli, 20 (2014), no 2, 676-696.
     ...
     Text
      The score equations of the log-likelihood function of a graphical model can be computed as follows:
     ...
     Text
      Next we compute the MLE for the covariance matrix of a @TO mixedGraph@ with undirected, directed and bidirected edges:
     ...
     Text
      @TO solverMLE@ can also be used to solve positive definite matrix completion problems. 
      For instance, to complete the following matrix
      ...
      into a PD matrix we give values to x and y and compute the MLE for the concentration matrix in the Gaussian graphical model 
      given by the 4-cycle. Note that not all values for x and y will result in a maximum likelihood estimate.
      Observe in the last line below that only the entries of the outputed matrix that correspond to non-edges of the graph
      differ from the entries of the input matrix. This comes from Example 12.16 in the book: Mateusz Michalek and Bernd Sturmfels,
...
doc ///
  ...
  Description  
    Text
     ...
     It might not provide the score equations but only the ideal before the saturation procedure.
...
doc ///
	...
	Text
	    In the following example we compute the MLE for the covariance matrix of a @TO mixedGraph@ with
	...
	Text
	   Next we provide the MLE for the concentration matrix of a @TO mixedGraph@ with undirected, directed and bidirected edges:
	...
	Text
	   Finally, @TO solverMLE@ can also be used to solve positive definite matrix completion problems. 
	   For instance, to complete the following matrix
