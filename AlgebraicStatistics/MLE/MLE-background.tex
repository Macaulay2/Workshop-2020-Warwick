...
\textcolor{red}{definition of statistical model}

\subsection{MLE and contingency tables}
Let $\mathcal{D}=\{X^{(1)},X^{(2)},\cdots,X^{(n)} \in \mathbb{R}^m\}$ be the data consisting of $n$ independent and identically distributed (i.i.d.) random vectors $X^{(i)}\sim P_\theta$.

\textcolor{red}{general procedure for MLE}
...
\textcolor{red}{Gaussian MLE}

\section{Graphical models}

From graph to data and back

graphical models give a parametrization of a CI model. but not all CI models admit a parametrization. (above question 3.1.9)
...
\subsection{Mixed/Chain models}
\textcolor{blue}{Are chain and mixed models the same thing? Mixed models are not mentioned in the references}
