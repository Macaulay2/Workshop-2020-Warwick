...
trial (ZZ,List) := List => (k,statements) ->
(
    l := #statements;
    s := {};
    for i from 0 to l-1 do
...
L#0
L1#0
L2 = apply(L,l->{toList apply(l_0,t->sort t),toList apply(l_1,t-> sort t)})
L2#0
isSubset(L1#0,L2)
L1#0
L2
for l in L2 do
(
    if l#0 == {{a,b,d},{a,c,d},{a,b,c,d}} then print l; 
)

for l in L1 do
(
    if not member(l,L2) then print l;    
)
